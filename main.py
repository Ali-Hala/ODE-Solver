...
# This is the 1NSpace file. This is the main file that solves ODEs in the 1N space. In order to better understand this code, start reading at __main__ found on line 150.
...
def get_constants():
    constants = {}
    while True:
        constant_input = input("Enter a constant (name=value), or press Enter to finish. You cannot only use letters in constant names except for t, y, x, P, and d: ")
...
def solve_ode(equation_str, num_solutions, initial_guess, increments, equation_str2, resolution, num_variables, equation_str3):
    ...
    ode_exprs = []
    y_symbols = symbols([f'y{j + 1}' for j in range(first_layer)])
    t = symbols('t')
    expr_strs = []
    ...
    for i, val in enumerate(K): # This loop defines the rest of the ODEs in our system using the galerkin matrix (K).
    ...
    y0 = [float(input(f"Enter initial condition for y{i + 1}: ")) for i in range(first_layer)]
